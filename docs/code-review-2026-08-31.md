...
| C9 | `UI/AppRulesPane.swift:76` | **已确认并修复**：自定义规则编辑器穷举 `ScrollSettings` 全部 10 个字段 |
...
**C9 复核结果**：结论真实。`ScrollSettings` 已有 10 个持久化字段，而应用级 custom 编辑器只提供平滑、垂直反转、最短步长、速度和平滑度 5 项，横向反转、加速度、连续设备反转/平滑和滚动阶段无法按应用配置。现由 `AppRuleScrollField.allCases` 驱动控件，并通过穷举 switch 绑定全部字段；自检把该枚举 raw value 与 `ScrollSettings` 实际 JSON 编码键集合比较，当前覆盖 10/10，未来任一侧增删字段都会失败。总计 230 项。
