...
enum Strings {
    ...
    static let scrollMinimumStepHelp = "单次滚动事件至少走多少像素。慢慢拨一格时的距离由它决定。"
    static let scrollSpeed = "速度增益"
    static let scrollSpeedHelp = "在系统上报的像素增量上再乘一个倍数。因为系统的增量本身会随滚动变快而变大，所以快速滚动会自然加速。"
    static let scrollSmoothness = "平滑度"
    static let scrollSmoothnessHelp = "每帧向目标推进的比例的反面：越大越顺滑绵长，越小越干脆跟手。"
    static let scrollAcceleration = "加速度"
    static let scrollAccelerationHelp = "连续快速滚动时额外放大距离，1.0 表示关闭。"
    static let scrollSectionTrackpad = "触控板与 Magic Mouse"
    static let scrollTrackpadNote = "这类设备本身就是像素级连续滚动，默认不做处理，以免破坏手势与惯性。"
    static let scrollReverseTrackpad = "同时反转它们的方向"
    static let scrollSmoothTrackpad = "也对它们做平滑处理"
